...
describe('create deck test', () => 
{
  ...
  it('Get workouts from local paths', async () => 
  {
    ...
    // Query select all <product-item> elements and return length of that array
    const numWorkouts = await page.$$eval('workout-card', (workoutCards) => 
    {
      return workoutCards.length;
    });
  
    // Expect there that array from earlier to be of length 43
  ...
  {
    console.log('Check <product-item> elements are populated...');
...
describe('create deck test', () => 
{
  ...
  it('should toggle selection mode and allow selecting a card', async () => 
  {
    ...
    const isSelected = await page.evaluate(
      (card) => card.classList.contains('selected'), firstCard
    );
  ...
  it('Error if Create Deck is clicked with no selection', async () => 
  {
    // Deselect all cards
    await page.evaluate(() => 
    {
    ...
    const errorText = await page.$eval(
      '#create-deck-error', (el) => el.textContent
    );
