...
describe("Delete Deck Functionality", () => {
  ...
  test("disables delete button for default decks", () => {
    localStorage.setItem("selected-deck", JSON.stringify({ name: "Chest Day" }));
    setupDeleteDeck();
  ...
  test("does not disable delete button for custom decks", () => {
    localStorage.setItem("selected-deck", JSON.stringify({ name: "My Custom Deck" }));
    setupDeleteDeck();
  ...
  test("clicking confirm deletes custom deck, hides modal, and shows deleted message", () => {
